...
class flightcog(commands.Cog):
    ...
    async def lookupflight(self,
        ...
        if depcountry:
            res = await to_thread(requests.get, f"{url}{'or'.join([f'country_name=%22{c}%22' for c in depcountry.split()])}")
            self.depcity = [f"{x.get('column_1')}.AIRPORT" for x in res.json().get("results")]
        else:
            res = await to_thread(requests.get, f"{url}{'or'.join([f'city_name=%22{c}%22%20or%20airport_name%20LIKE%20%22{c}%22' for c in depcity.split()])}")
            self.depcity = [f"{x.get('column_1')}.AIRPORT" for x in res.json().get("results")]
        if arrcountry:
            res = await to_thread(requests.get, f"{url}{'or'.join([f'country_name=%22{c}%22' for c in arrcountry.split()])}")
            self.arrcity = [f"{x.get('column_1')}.AIRPORT" for x in res.json().get("results")]
        else:
            res = await to_thread(requests.get, f"{url}{'or'.join([f'city_name=%22{c}%22%20or%20airport_name%20LIKE%20%22{c}%22' for c in arrcity.split()])}")
            self.arrcity = [f"{x.get('column_1')}.AIRPORT" for x in res.json().get("results")]
        self.vacmin = vacmin
        self.vacmax = vacmax
