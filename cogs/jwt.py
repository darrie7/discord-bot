...
class Torrent:
    ...
    async def media_scraper(self):
        ...
        while n < 2:
            ...
            title_magnet = [{'title': item.find('title').text, 'magnet': item.find('link').text} for item in root.findall('.//item') if (not any(word in item.find('title').text.lower() for word in ['hdrip', 'camrip', 'hdcam', 'hdts']) and int(item.find(".//{http://torznab.com/schemas/2015/feed}attr[@name='seeders']").attrib['value']) > 2 and self.db_entry.get('title').lower() in self.guess_title(item) )]
...
class justwatchCog(commands.Cog):
    ...
    async def searchmedia(self) -> None:
        ...
        await self.bot.get_channel(793878235066400809).send(f"""```{[ x.get('title') for x in data ]}'''""")
